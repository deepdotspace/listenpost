...
export const actions: Record<string, ActionHandler<Env>> = {
  /**
   * Generate a data-layer API key. The raw key is returned ONCE and never
   * stored — only its SHA-256 hash. Runs as a server action because
   * clients can't create api_keys rows (RBAC: create false).
   */
  generateApiKey: async ({ userId, params, tools }) => {
